...
def projection_by_bboxes(boxes: np.array, axis: int) -> np.ndarray:
    ...
    """
    assert axis in [0, 1]
    length = np.max(boxes[:, axis::2])   # box涉及到最大像素值
    print("length: ", length)

    res = np.zeros(length, dtype=int)

    # TODO: how to remove for loop?
    print("boxes[:, axis::2]: ", boxes[:, axis::2])
...
def recursive_xy_cut(boxes: np.ndarray, indices: List[int], res: List[int]):
    ...
    _indices = boxes[:, 1].argsort()  # 根据左上角y坐标，对box进行排列
    ...
    for r0, r1 in zip(arr_y0, arr_y1):
        ...
        _indices = y_sorted_boxes_chunk[:, 0].argsort()  # 在一个局部block中，根据x坐标进行排序
